...
\runningheader{}{}{}
\firstpagefooter{}{}{}
\runningfooter{ACM/CS 114}{Assignment 2 - due 29 Jan 2010}{\thepage}
...
%
  ...
  $z > 1$ where the dilogarithm picks up an imaginary part.
%
...
%
  \part Write a driver for \function{dilog} and convince yourself that it gives
  reasonable approximations to the values in \eqref{li1} and \eqref{li-1}. Fill out the
  following table:
  \begin{table}[h]
    \small
    \centering
...
\question Implement a version of \function{dilog} that uses pthreads for enhanced performance.
\begin{parts}
  ...
  \part Implement \function{dilog} to perform the error checking, spawn $n$ threads to perform
  the calculation, collect the results of each thread and return the approximation to the
  integral.
  \part Convince yourself that \function{dilog} returns correct values regardless of the number
  of threads it spawned.
\end{parts}

\question Implement a parallel version \function{dilog} using \mpi.
\begin{parts}
  \part What is the correct partitioning, coarsening and mapping strategies when using
  distributed memory parallelism?
  \part Implement \function{dilog}, making sure that each processor has the correct return
