...
struct context {
    ...
    double partial;     // a record of the partial sum computed by this thread
...
double dilog(double z, long N, int threads) {
    ...
    for (int tid=0; tid<threads; tid++) {
        ...
        // create the thread
        int status = pthread_create(&(thr_info[tid].descriptor), 0, worker, &thr_info[tid]);
        if (status) {
...
int main(int argc, char* argv[]) {
    ...
    // abort if N < 1
    if (N < 1) {
        std::cout << "the number of subdivisions must be positive" << std::endl;
        return 0;
    }
