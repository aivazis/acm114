...
\lecture{Programming with pthreads - part 2}{20100120}
...
int main(int argc, char* argv[]) {
    ...
    for (id=0; id<THREADS; id++) {
        ...
        status = pthread_create(&threads[id], NULL, squares, (void*) id);
        if (status) {
...
\begin{frame}[fragile]
%
  \frametitle{A reduction using threads}
  \label{slide:squares}
%
  \begin{C}
#include <pthread.h>
...
int main(int argc, char* argv[]) {
    ...
    for (id=0; id<THREADS; id++) {
        pthread_create(&threads[id], NULL, hello, (void*) id);
    }
    pthread_exit(NULL);
    return 0;
}
